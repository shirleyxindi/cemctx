...
def epistemic_search(
    ...
    *,
    ...
    extra_data: Any = None,
    loop_fn: base.EpistemicLoopFn = jax.lax.fori_loop
) -> EpistemicTree:
    ...
    def body_fun(sim, loop_state):
        ...
        next_node_index = jnp.where(
            next_node_index == EpistemicTree.UNVISITED, sim + 1, next_node_index
        )
        tree = expand(
            params,
            expand_key,
...
def expand(
    ...
    tree = update_tree_node(
        ...
        step.prior_logits,
        step.value,
        step.value_epistemic_variance,
        cumulative_cost,
        step.cost_value,
        step.cost_value_epistemic_variance,
        embedding,
    )
    ...
    return tree.replace(
        ...
        children_rewards_epistemic_variance=batch_update(
            tree.children_rewards_epistemic_variance,
            step.reward_epistemic_variance,
            parent_index,
            action,
        ),
        ...
        children_costs_epistemic_variance=batch_update(
            tree.children_costs_epistemic_variance,
            step.cost_epistemic_variance,
            parent_index,
            action,
        ),
    )
...
@jax.vmap
def backward(tree: EpistemicTree[T], leaf_index: chex.Numeric) -> EpistemicTree[T]:
    ...
    def body_fun(loop_state):
        ...
        parent_value_epistemic_std = (
            tree.node_values_epistemic_std[parent] * count
            + jnp.sqrt(leaf_value_epistemic_variance)
        ) / (count + 1.0)
        ...
        leaf_cost_value = (
            cost + tree.children_discounts[parent, action] * leaf_cost_value
        )
        parent_cost_value = (
            tree.node_cost_values[parent] * count + leaf_cost_value
        ) / (count + 1.0)
        ...
        parent_cost_value_epistemic_std = (
            tree.node_cost_values_epistemic_std[parent] * count
            + jnp.sqrt(leaf_cost_value_epistemic_variance)
        ) / (count + 1.0)
        ...
        tree = tree.replace(
            ...
            node_values_epistemic_std=update(
                tree.node_values_epistemic_std, parent_value_epistemic_std, parent
            ),
            ...
            children_values_epistemic_std=update(
                tree.children_values_epistemic_std,
                children_values_epistemic_std,
                parent,
                action,
            ),
            ...
            node_cost_values=update(tree.node_cost_values, parent_cost_value, parent),
            node_cost_values_epistemic_std=update(
                tree.node_cost_values_epistemic_std,
                parent_cost_value_epistemic_std,
                parent,
            ),
            ...
            children_cost_values_epistemic_std=update(
                tree.children_cost_values_epistemic_std,
                children_cost_values_epistemic_std,
                parent,
                action,
            ),
        )
    ...
    loop_state = (
        tree,
        tree.node_values[leaf_index],
        jnp.square(tree.node_values_epistemic_std[leaf_index]),
        tree.node_cost_values[leaf_index],
        jnp.square(tree.node_cost_values_epistemic_std[leaf_index]),
        leaf_index,
    )
    tree = jax.lax.while_loop(cond_fun, body_fun, loop_state)[0]
...
def update_tree_node(
    ...
    updates = dict(  # pylint: disable=use-dict-literal
        ...
        node_values_epistemic_std=batch_update(
            ...
            tree.node_values_epistemic_std,
            jnp.sqrt(value_epistemic_variance),
            node_index,
        ),
        raw_cost_values=batch_update(tree.raw_cost_values, cost_value, node_index),
        raw_cost_values_epistemic_variance=batch_update(
            tree.raw_cost_values_epistemic_variance,
            cost_value_epistemic_variance,
            node_index,
        ),
        node_cost_values=batch_update(tree.node_cost_values, cost_value, node_index),
        node_cost_values_epistemic_std=batch_update(
            tree.node_cost_values_epistemic_std,
            jnp.sqrt(cost_value_epistemic_variance),
            node_index,
        ),
...
def instantiate_tree_from_root(
    ...
    tree = EpistemicTree(
        ...
        children_rewards_epistemic_variance=jnp.zeros(
            batch_node_action, dtype=data_dtype
        ),
        ...
        node_cumulative_costs=jnp.zeros(batch_node, dtype=data_dtype),
        ...
        children_costs_epistemic_variance=jnp.zeros(
            batch_node_action, dtype=data_dtype
        ),
        children_cost_values_epistemic_std=jnp.zeros(
            batch_node_action, dtype=data_dtype
        ),
        beta_c=beta_c,
    ...
    tree = update_tree_node(
        ...
        root.prior_logits,
        root.value,
        root.value_epistemic_variance,
        root_cumulative_cost,
        root.cost_value,
        root.cost_value_epistemic_variance,
        root.embedding,
    )
    return tree
