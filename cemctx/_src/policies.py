...
from cemctx._src import qtransforms
from cemctx._src import search
from cemctx._src import seq_halving
from cemctx._src import epistemic_search


def epistemic_muzero_policy(
    params: base.Params,
    ...
    *,
...
) -> base.EpistemicPolicyOutput[None]:
    ...
    interior_action_selection_fn = functools.partial(
        action_selection.epistemic_muzero_action_selection,
        pb_c_base=pb_c_base,
        pb_c_init=pb_c_init,
        qtransform=qtransform,
    )
    root_action_selection_fn = functools.partial(interior_action_selection_fn, depth=0)
    search_tree = epistemic_search.epistemic_search(
        params=params,
    ...
    return base.EpistemicPolicyOutput(
...
def epistemic_gumbel_muzero_policy(
    params: base.Params,
    rng_key: chex.PRNGKey,
    root: base.EpistemicRootFnOutput,
    recurrent_fn: base.EpistemicRecurrentFn,
    ...
    *,
...
) -> base.EpistemicPolicyOutput[action_selection.GumbelMuZeroExtraData]:
    ...
    search_tree = epistemic_search.epistemic_search(
        ...
        root_action_selection_fn=functools.partial(
            action_selection.epistemic_gumbel_muzero_root_action_selection,
            num_simulations=num_simulations,
            max_num_considered_actions=max_num_considered_actions,
            qtransform=qtransform,
        ),
        interior_action_selection_fn=functools.partial(
            action_selection.epistemic_gumbel_muzero_interior_action_selection,
            qtransform=qtransform,
        ),
        num_simulations=num_simulations,
        max_depth=max_depth,
    ...
    completed_qvalues = jax.vmap(qtransform, in_axes=[0, None])(
        # pytype: disable=wrong-arg-types  # numpy-scalars  # pylint: disable=line-too-long
        search_tree,
        search_tree.ROOT_INDEX,
    )
    ...
    # TODO: This action should probably be shielded, we are using shielding in the select phase so technically the idea that unsafe actions are taken less often is encoded in the visitation frequencies, but still we should probably use the final estimates for the q_cost values to shield the final action selection
...
def _make_stochastic_recurrent_fn(
    ...
    def stochastic_recurrent_fn(
        ...
        chance_output, state_embedding = chance_node_fn(
            params, rng, chance_outcome, state.afterstate_embedding
        )
        ...
        def _broadcast_where(decision_leaf, chance_leaf):
            ...
            expanded_is_decision = jnp.reshape(
                state.is_decision_node, [-1] + extra_dims
            )
            return jnp.where(
        ...
        output = jax.tree.map(
            _broadcast_where, output_if_decision_node, output_if_chance_node
        )
...
def _make_stochastic_action_selection_fn(
    ...
    def _action_selection_fn(
        key: chex.PRNGKey, tree: search.Tree, node_index: chex.Array, depth: chex.Array
    ) -> chex.Array:
        is_decision = tree.embeddings.is_decision_node[node_index]
        chance_selection = (
            _chance_node_selection_fn(
                tree=_mask_tree(tree, num_actions, "chance"), node_index=node_index
            )
            + num_actions
        )
        ...
        return jax.lax.cond(
            is_decision, lambda: decision_selection, lambda: chance_selection
        )
